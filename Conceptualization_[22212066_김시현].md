...
| 05/05/2026 | 0.02 | Refined concept based on Analysis (Items added) | 김시현 |
---
...
```text
Game Connection: 게임 접속
Set Guest Nickname: 게스트 닉네임 설정
Create Room: 방 만들기 (호스트)
Join Room: 방 참여 (클라이언트)
Move Character: 캐릭터 이동 (플레이어 조작)
Transfer Bomb: 폭탄 전달 (상호작용 키 입력)
Acquire Item: 아이템 획득 (버프 효과 적용)
...
| Actor | Player |
| --- | --- |
| Description | 유저가 로그인을 할 지 게스트로 참여할 지를 정한다. |

**2) 게스트 닉네임 설정**

| Actor | Player |
|---|---|
| Description | 게스트로 참여한 유저는 자신이 사용할 닉네임을 정한다. |
...
| Actor | Plyaer |
|---|---|
| Description | 플레이어가 맵 상의 아이템과 충돌하여 즉시 버프를 받는다. |
...
| Purpose | 유저 식별 및 멀티플레이 세션 진입 준비. |
| --- | --- |
| Approach | UI에서 닉네임을 입력받아 저장. |
| Dynamics | 게스트로 참여할 시. |
| Goals | 혼선 없는 고유 플레이어 프로필 생성. |

**3) 방 만들기 (Host Session)**
...
| Purpose | 버프 효과를 획득하여 도주 및 추격 |
| --- | --- |
| Approach | 맵 곳곳에 아이템이 떨어짐. |
| Dynamics | 떨어진 아이템과 충돌 시 버프 효과 획득. |
| Goals | 버프 효과로 추격 및 도주에 변수를 창출. |
